...
const getCurrentUser = getSafeController(async (req: RequestWithUser, res: Response) => {
  const { id } = req.user;
  const user = await userService.getUsersByIds([id]);
  res.status(200).json(user);
});
...
const updateUser = getSafeController(async (req: RequestWithUser, res: Response) => {
...
const deleteUser = getSafeController(async (req: RequestWithUser, res: Response) => {
